...
public:
    OpticalFlowTracker(
        ...
        bool inverse_ = true, bool has_initial_ = false) : img1(img1_), img2(img2_), kp1(kp1_), kp2(kp2_),
...
inline double GetPixelValue(const cv::Mat &img, float x, float y) {
    ...
    x = max(x, 0.f);
    y = max(y, 0.f);
    x = min(x, static_cast<float>(img.cols - 1));
    y = min(y, static_cast<float>(img.rows - 1));
    ...
    return (1 - xx) * (1 - yy) * img.at<uchar>(floor(y), floor(x))
           + xx * (1 - yy) * img.at<uchar>(floor(y), x_a1)
           + (1 - xx) * yy * img.at<uchar>(y_a1, floor(x))
...
void OpticalFlowSingleLevel(
    ...
    cv::parallel_for_(cv::Range(0, static_cast<int>(kp1.size())),
                      [&tracker](auto &&PH1) { tracker.calculateOpticalFlow(std::forward<decltype(PH1)>(PH1)); });
}
